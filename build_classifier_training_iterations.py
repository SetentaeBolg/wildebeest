...
not_wildebeest.to_csv('new_not_wildebeest_locations.csv', header=True, mode='w')
...
for i in range(df.size):
	...
	for y_pos in range(0, img.size[1], ny):
		for x_pos in range(0, img.size[0], nx):
			...
			# predicted_class = np.concatenate(np.concatenate(predicted_class, predicted_class, axis=2), predicted_class, axis=2)
	...
	not_wildebeest.to_csv('new_not_wildebeest_locations.csv', header=False, mode='a')
